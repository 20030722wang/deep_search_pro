...
def generate_markdown(
    ...
    logger.debug("generate_markdown session_dir=%s", session_dir)
    ...
    # 确保目录存在
    logger.debug("[MarkdownTool] parent_dir=%s, filename=%s, full_path=%s", parent_dir, filename, file_path)

    try:
        if not parent_dir.exists():
            parent_dir.mkdir(parents=True, exist_ok=True)
            logger.info("[MarkdownTool] Created directory: %s", parent_dir)
