...
# 延迟初始化：避免 import 时因环境变量缺失或服务不可达而崩溃
...
def get_assistant_list() -> str:
    ...
    try:
        ...
        for chat in chat_list:
            dataset_names = []
            dataset_ids = chat.dataset_ids  # 当前聊天助手关联的知识库ID列表
            if dataset_ids and isinstance(dataset_ids, list):
                for ds_id in dataset_ids:
                    # 通过ID查询知识库详情以获取名称
                    try:
                        ds_list = _get_ragflow_client().list_datasets(id=ds_id)
                        if ds_list:
                            dataset_names.append(ds_list[0].name)
                    except Exception:
                        pass  # 单个知识库查询失败不影响整体
